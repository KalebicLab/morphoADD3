...
raw = IJ.openImage( filename.getPath() )
IJ.run(raw, "8-bit", "");
project = ZProjector()
project.setMethod(ZProjector.MAX_METHOD)
project.setImage(raw) #imageplus
project.doProjection()
imp0 = project.getProjection()


IJ.setRawThreshold(imp0, 1, 255);
...
if saveResults :
	...
	saveNameCellInfo = os.path.join( savePath, basename+"_CellInfo.xls" )
	#IJ.selectWindow( rt_cellInfo_Name )
	#IJ.saveAs("Results", saveNameCellInfo )
	rt_cellInfo.saveAs(saveNameCellInfo)
	
	saveNameProcessInfo = os.path.join( savePath, basename+"_ProcessInfo.xls" )
	...
	savenameResultImage = os.path.join( savePath, basename+"_results.tif" )
